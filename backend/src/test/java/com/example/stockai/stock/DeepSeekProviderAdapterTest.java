...
class DeepSeekProviderAdapterTest {
    ...
    void parsesStructuredAnalysisFromChatCompletion() {
        String body = """
            {
              "choices": [
                {
                  "message": {
                    "content": "{\\"trend\\":\\"偏多\\",\\"aiScore\\":77,\\"bullishProbability\\":0.68,\\"riskLevel\\":\\"MEDIUM\\",\\"summary\\":\\"短線動能穩定。\\"}"
                  }
                }
              ]
            }
        ...
        assertThat(result.orElseThrow().provider()).isEqualTo("DEEPSEEK");
        assertThat(result.orElseThrow().aiScore()).isEqualTo(77);
        assertThat(result.orElseThrow().source()).isEqualTo("deepseek-chat-completions");
    }

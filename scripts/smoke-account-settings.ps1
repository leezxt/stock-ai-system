...
$register = Invoke-RestMethod -Uri "$baseUrl/auth/register" -Method Post -ContentType "application/json" -Body $authBody -TimeoutSec 15
if (-not $register.data.token) {
  throw "Register did not return token."
}

$login = Invoke-RestMethod -Uri "$baseUrl/auth/login" -Method Post -ContentType "application/json" -Body $authBody -TimeoutSec 15
...
$initial = Invoke-RestMethod -Uri "$baseUrl/account/settings" -Headers $headers -TimeoutSec 15
...
$saved = Invoke-RestMethod -Uri "$baseUrl/account/settings" -Method Put -Headers $headers -ContentType "application/json" -Body $saveBody -TimeoutSec 15
...
$partial = Invoke-RestMethod -Uri "$baseUrl/account/settings" -Method Put -Headers $headers -ContentType "application/json" -Body $partialBody -TimeoutSec 15
...
$cleared = Invoke-RestMethod -Uri "$baseUrl/account/settings" -Method Put -Headers $headers -ContentType "application/json" -Body $clearBody -TimeoutSec 15
...
$loaded = Invoke-RestMethod -Uri "$baseUrl/account/settings" -Headers $headers -TimeoutSec 15
